...
RUN apt-get update && apt-get upgrade -y && apt-get autoremove -y \
    && apt-get install -y -qq git libmcrypt-dev libpng12-dev libjpeg-dev libpq-dev mysql-client curl \
...
RUN usermod -u 1000 www-data && groupmod -g 1000 www-data && usermod -g 1000 www-data && apt-get purge -y --auto-remove && apt-get clean
